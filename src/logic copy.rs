...
pub fn render_lights(s: &mut State, l: &mut Lights) {
    ...
    for (i, fr) in l.dimmers.iter_mut().enumerate() {
        *fr = if i == target { 1.0 } else { 0.0 };
    }
...
pub fn render_pad(s: &mut State, lights: &Lights, pad: &mut Midi<LaunchpadX>) {
    use self::Mode;
    ...
    for (n, fr) in lights.dimmers.iter().enumerate() {
        let x = n % 3;
        let y = n / 3;
...
pub fn render_ctrl(s: &mut State, ctrl: &mut Midi<LaunchControlXL>) {
    ...
    set(3, matches!(s.palette, Palette::Skullsystem));
}
...
pub fn on_ctrl(s: &mut State, l: &mut Lights, ctrl: &mut Midi<LaunchControlXL>, input: launch_control_xl::Input) {
    ...
    match input {
        ...
        Input::Control(3, true) => s.palette = Palette::Skullsystem,
