...
pub struct State {
    ...
    pub phi_mul: f64,
    /// Second stacking bpm multiplier.
    pub phi_mul2: f64,
    /// Strobe duty cycle
    pub duty: f64,
...
impl Palette {
    pub fn colors(&self, i: usize, j: usize) -> Colors {
        match self {
            Palette::Standard => match (i, j) {
                // Whitez
                (0, 0) => Colors::Solid(Rgbw::WHITE),
                (0, 1) => Colors::Solid(Rgbw::RGBW),

                // Primariez
                (1, 0) => Colors::Solid(Rgbw::RED),
                (1, 1) => Colors::Split(Rgbw::RED, Rgbw::WHITE),
                (2, 0) => Colors::Solid(Rgbw::LIME),
                (2, 1) => Colors::Split(Rgbw::LIME, Rgbw::WHITE),
                (3, 0) => Colors::Solid(Rgbw::BLUE),
                (3, 1) => Colors::Split(Rgbw::BLUE, Rgbw::WHITE),

                // nBluez
                (4, 0) => Colors::Split(Rgbw::BLUE, Rgbw::LIME),
                (4, 1) => Colors::Split(Rgbw::RED, Rgbw::BLUE),

                // Rainbowz
                (5, 0) => Colors::RedWhiteOsc,
                (5, 1) => Colors::BlueGreenOsc,
                (6, 0) => Colors::RgbOsc,
                (6, 1) => Colors::RainbowOsc,

                _ => Colors::Solid(Rgbw::WHITE),
            },
            Palette::Trance => match (i, j) {
                (1, 0) => Colors::Solid(Rgbw::BLUE),
                (1, 1) => Colors::Solid(Rgbw::CYAN),
                (2, 0) => Colors::Solid(Rgbw::LIME),
                (2, 1) => Colors::Solid(Rgbw::MINT),
                (3, 0) => Colors::Solid(Rgbw::PEA),
                (3, 1) => Colors::Solid(Rgbw::MAGENTA),
                _ => Palette::Standard.colors(i, j),
            },
            Palette::Skullsystem => match (i, j) {
                (1, 0) => Colors::Solid(Rgbw::BLUE),
                (1, 1) => Colors::Solid(Rgbw::CYAN),
                (2, 0) => Colors::Solid(Rgbw::LIME),
                (2, 1) => Colors::Solid(Rgbw::MINT),
                (3, 0) => Colors::Solid(Rgbw::VIOLET),
                (3, 1) => Colors::Solid(Rgbw::MAGENTA),
                _ => Palette::Standard.colors(i, j),
            },
            Palette::Redhot => match (i, j) {
                (1, 0) => Colors::Solid(Rgbw::RED),
                (1, 1) => Colors::Split(Rgbw::RED, Rgbw::WHITE),
                (2, 0) => Colors::Solid(Rgbw::RED),
                (2, 1) => Colors::Split(Rgbw::WHITE, Rgbw::RED),
                (3, 0) => Colors::Solid(Rgbw::VIOLET),
                (3, 1) => Colors::Solid(Rgbw::MAGENTA),
                _ => Palette::Standard.colors(i, j),
            },
            Palette::Ashton1 => match (i, j) {
                // Whitez
                (0, 0) => Colors::Solid(Rgbw::WHITE),
                (0, 1) => Colors::Solid(Rgbw::RGBW),
                ...
                (3, 0) => Colors::Solid(Rgbw::BLUE),
                (3, 1) => Colors::Split(Rgbw::BLUE, Rgbw::WHITE),
                (4, 0) => Colors::Split(Rgbw::YELLOW, Rgbw::WHITE),
                (4, 1) => Colors::Split(Rgbw::YELLOW, Rgbw::BLUE),

                // Rainbowz
                (5, 0) => Colors::RedWhiteOsc,
                (5, 1) => Colors::BlueWhiteOsc,
                (6, 0) => Colors::GreenWhiteOsc,
                (6, 1) => Colors::RgbOsc,

                _ => Colors::Solid(Rgbw::WHITE),
            },
            Palette::Ashton2 => match (i, j) {
                // Whitez
                (0, 0) => Colors::Solid(Rgbw::WHITE),
                (0, 1) => Colors::Solid(Rgbw::RGBW),
                ...
                // Primariez
                (1, 0) => Colors::Solid(Rgbw::RED),
                (1, 1) => Colors::Solid(Rgbw::ORANGE),
                (2, 0) => Colors::Solid(Rgbw::LIME),
                (2, 1) => Colors::Solid(Rgbw::YELLOW),
                (3, 0) => Colors::Solid(Rgbw::BLUE),
                (3, 1) => Colors::Solid(Rgbw::CYAN),
                (4, 0) => Colors::Split(Rgbw::BLUE, Rgbw::LIME),
                (4, 1) => Colors::Split(Rgbw::BLUE, Rgbw::YELLOW),

                // Rainbowz
                (5, 0) => Colors::BlueGreenOsc,
                (5, 1) => Colors::BlueYellowOsc,
                (6, 0) => Colors::RainbowOsc,
                (6, 1) => Colors::RgbOsc,
...
impl Colors {
    fn color0(self, s: &mut State, fr: f64) -> Rgbw {
        match self {
            Colors::Rainbow => Rgb::hsv(s.phi(16, 1), 1.0, 1.0).into(),
            Colors::RgbOsc => match s.pd(Pd(1, 2)).ramp(1.0) {
                ..0.33 => Rgbw::RED,
                0.33..0.66 => Rgbw::LIME,
                _ => Rgbw::BLUE,
            },
            Colors::RainbowOsc => match (s.pd(Pd(1, 2)).ramp(1.0) * 8.0).floor() as u8 {
                0 => Rgbw::RED,
                1 => Rgbw::ORANGE,
                2 => Rgbw::YELLOW,
                3 => Rgbw::LIME,
            ...
            Colors::RedWhiteOsc => match s.pd(Pd(1, 2)).ramp(1.0) {
                ..0.5 => Rgbw::RED,
                _ => Rgbw::WHITE,
            },
            Colors::BlueGreenOsc => match s.pd(Pd(1, 2)).ramp(1.0) {
                ..0.5 => Rgbw::BLUE,
                _ => Rgbw::LIME,
            },
            Colors::BlueYellowOsc => match s.pd(Pd(1, 2)).ramp(1.0) {
                ..0.5 => Rgbw::BLUE,
                _ => Rgbw::YELLOW,
            },
            Colors::BlueWhiteOsc => match s.pd(Pd(1, 2)).ramp(1.0) {
                ..0.5 => Rgbw::BLUE,
                _ => Rgbw::WHITE,
            },
            Colors::GreenWhiteOsc => match s.pd(Pd(1, 2)).ramp(1.0) {
                ..0.5 => Rgbw::LIME,
                _ => Rgbw::WHITE,
            },
            Colors::Solid(col) => col,
            Colors::Split(col0, _col1) => col0,
        }
    }

    fn color1(self, s: &mut State, fr: f64) -> Rgbw {
        match self {
            Colors::Split(_col0, col1) => col1,
            _ => self.color0(s, fr),
        }
    }
}
...
pub fn render_lights(s: &mut State, l: &mut Lights) {
    ...
    match s.mode {
        ...
        Mode::On { beams, spiders } => {
            l.split(s.colors.color0(s, 0.0), s.colors.color1(s, 0.0));

            if let Some(beams) = beams {
                let col = s.colors.color1(s, 0.0);
                l.for_each_beam(|beam, i, fr| {
                    beams.apply(s, Pd(4, 1), beam, i, fr);
                    beam.color = col;
                });
            }
            if let Some(spiders) = spiders {
                l.for_each_spider(|sp, i, fr| spiders.apply(s, sp, i, fr));
            }
        }
        Mode::AutoBeat { pd, r, beam: beam_pattern } => {
            ...
            l.split(s.colors.color0(s, 0.0) * env, s.colors.color1(s, 0.0) * env);
        ...
        Mode::Strobe { pd, duty } => {
            ...
            l.strobe.color = Rgb::from(s.colors.color0(s, 0.0) * env);
        }
        Mode::Strobe0 { pd, duty } => {
            let p = s.colors;
            let env = s.pd(pd.mul(2)).square(1.0, duty.in_exp().lerp(1.0..0.5));
            l.split(s.colors.color0(s, 0.0) * env, Rgbw::BLACK);
        ...
        Mode::Strobe1 { pd, duty } => {
            let p = s.colors;
            let env = s.pd(pd.mul(2)).square(1.0, duty.in_exp().lerp(1.0..0.5));
            l.split(Rgbw::BLACK, s.colors.color0(s, 0.0) * env);
        ...
        Mode::ChaseSmooth { pd, beam: beam_pattern } => {
            ...
            l.for_each_spider(|sp, i, fr| {
                sp.color0 = color;
                sp.color1 = color;
                SpiderPattern::Alternate { pd: Pd(2, 1) }.apply(s, sp, i, fr)
            });
        }
        Mode::ChaseColor { pd, beam: beam_pattern } => {
            let color0 = s.colors.color0(s, 0.0);
            let color1 = s.colors.color1(s, 0.0);

            l.for_each_par(|par, i, fr| par.color = color0 * s.pd(pd.mul(4)).phase(1.0, fr).square(1.0, 0.1));
            l.for_each_beam(|beam, i, fr| {
                beam.color = color1 * s.pd(pd.mul(4)).phase(1.0, fr).square(1.0, 0.1);
                beam_pattern.apply(s, Pd(1, 2), beam, i, fr);
            });
            l.strobe.color = Rgb::from(color1) * s.pd(pd.mul(4)).phase(1.0, 0.0).square(1.0, 0.1);
        }
        Mode::ChaseNotColorful { pd } => {
            let col0 = s.colors.color0(s, 0.0);
            let col1 = s.colors.color1(s, 0.0);
    ...
    l.laser.color = LaserColor::from_rgb(s.colors.color0(s, 0.0).into());
    ...
    // l.syn.set_slider(0, s.t.fsin(1.0));
    // l.syn.set_color(0, Rgb::from(s.colors.color0(s, 0.0)));
}
...
pub fn render_pad(s: &mut State, pad: &mut Midi<LaunchpadX>) {
    ...
    let brightness = s.brightness.lerp(0.25..1.0);

    // Helper to set an x/y coord to a certain color
    let rgb = |Rgb(r, g, b): Rgb| Color::Rgb(r, g, b);
    let mut set = |x, y, color: Rgb| batch.push((Coord(x, y).into(), rgb(color * brightness)));

    if s.debug {
        let color0: Rgb = s.colors.color0(s, 0.0).into();
        let color1: Rgb = s.colors.color1(s, 0.0).into();
        ...
        set(4, 5, color1 * beat132);
        set(5, 5, Rgb::WHITE * beat116);
        set(6, 5, Rgb::WHITE * beat132);
        set(7, 5, Rgb::WHITE * beat132);

        // y=6, y=7: Colorz
        let mut palette = |i, j| Rgb::from(s.palette.colors(i, j).color0(s, 0.0));
        // Sidez
        set(0, 6, palette(0, 0));
        set(7, 6, palette(0, 1));
        // Primariez
        ...
        if let Some(ManualBeat { t0, t1, pd0, pd1, r }) = s.beat {
            for i in 0..8 {
                for j in 0..8 {
                    ...
                    //let col0 = s.palette.color0(s, 0.0) * (s.phi - i as f64 * 0.125).fsin(2.0).inout_exp();
                    //let col1 = s.palette.color0(s, 0.0) * (s.phi - i as f64 * 0.125).fsin(2.0).inout_exp();

                    set(i, j, Rgb::from(s.colors.color0(s, 0.0)) * fr);
                }
            }
        } else {
            match s.mode {
                Mode::On { .. } => {
                    let color = s.colors.color0(s, 0.0);
                    for i in 0..8 {
                        for j in 0..8 {
                            set(i, j, color.into());
                        }
                ...
                Mode::AutoBeat { pd, .. } => {
                    match s.x {
                        1 => {
                            // Upwards propagating wave at BPM
                            for i in 0..8 {
                                let col = s.colors.color0(s, 0.0) * (s.phi - i as f64 * 0.125).fsin(2.0).inout_exp();
                                for j in 0..8 {
                                    set(j, i, col.into());
                                }
                            }
                        ...
                        3 => {
                            ...
                            for i in 0..8 {
                                for j in 0..8 {
                                    let col = s.colors.color0(s, 0.0)
                                        * (s.phi - i as f64 * 0.125 + j as f64 * 0.125).fsin(2.0).in_quad();
                                    set(i, j, col.into());
                                }
                            }
                        }
                        ...
                        5 => {
                            ...
                            for x in 0..8 {
                                for y in 0..8 {
                                    set(x, y, Rgb::from(s.colors.color0(s, 0.0)) * spiral(s.t, x, y, -8.0));
                                }
                            }
                        }
                        _ => {
                ...
                Mode::RaisingBeams { .. } | Mode::Break { beams: Some(BeamPattern::WaveY | BeamPattern::UpDownWave) } => {
                    // Up/down wave
                    // let y0 = (.floor() as i8;
                    for x in 0..8 {
                        for y in 0..8 {
                            ...
                            set(x, y, Rgb::from(s.colors.color0(s, 0.0)) * (1.0 - env));
                            // if y == y0 {
                ...
                Mode::Strobe { pd, duty } | Mode::Strobe0 { pd, duty } | Mode::Strobe1 { pd, duty } => {
                    ...
                    let col = Rgb::from(s.colors.color0(s, 0.0)) * env;
                ...
                Mode::ChaseNotColorful { pd, .. } => {
                    let col = Rgb::from(s.colors.color0(s, 0.0));
                    for x in 0..8 {
                        for y in 0..8 {
    ...
    set(
        ...
        match s.pd(Pd(1, 1)).bsquare(1.0, 0.1) {
            true => match s.pd(Pd(4, 1)).bsquare(1.0, 0.2) {
                ...
                true => Rgb::WHITE,
                // Color0 on every other beat
                false => s.colors.color0(s, 0.0).into(),
            },
            false => Rgb::BLACK,
        },
    );
...
pub fn render_ctrl(s: &mut State, ctrl: &mut Midi<LaunchControlXL>) {
    ...
    set(0, matches!(s.palette, Palette::Standard));
    set(1, matches!(s.palette, Palette::Trance));
    set(2, matches!(s.palette, Palette::Redhot));
    set(3, matches!(s.palette, Palette::Ashton1));
    set(4, matches!(s.palette, Palette::Ashton2));
    set(5, matches!(s.palette, Palette::Skullsystem));
}
...
pub fn tick(dt: f64, s: &mut State, l: &mut Lights) {
    ...
    let random_colors = || match ThreadRng::default().gen_range(1..=10) {
        ...
        3 => Colors::Solid(Rgbw::BLUE),
        4 => Colors::Solid(Rgbw::CYAN),
        5 => Colors::Solid(Rgbw::LIME),
        6 => Colors::Solid(Rgbw::MINT),
        7 => Colors::Solid(Rgbw::PEA),
        8 => Colors::Solid(Rgbw::MAGENTA),
        9 | 10 | _ => Colors::Rainbow,
    };
    ...
    if s.preset {
        ...
        if phi % 32 == 0 {
            if !s.preset_switched {
                ...
                randomize(&mut s.colors, random_colors);
...
pub fn on_pad(s: &mut State, l: &mut Lights, pad: &mut Midi<LaunchpadX>, event: launchpad_x::Input) {
    ...
    match event {
        ...
        // Brightness
        Input::Record(true) => s.brightness = 0.0,
        Input::Solo(true) => s.brightness = 0.07,
    ...
    if let Some((x, y)) = match event {
        ...
        match (x, y) {
            ...
            // y=1: Solid patterns
            (1, 1) => s.mode = Mode::On { beams: Some(BeamPattern::Out), spiders: None },
            (2, 1) => s.mode = Mode::On { beams: Some(BeamPattern::Out), spiders: None },
            (3, 1) => {
                s.mode = Mode::On {
                    beams: Some(BeamPattern::WaveY),
                    spiders: Some(SpiderPattern::Alternate { pd: Pd(2, 1) }),
                }
            }
            (4, 1) => {
                s.mode = Mode::On {
                    beams: Some(BeamPattern::SnapX),
                    spiders: Some(SpiderPattern::Alternate { pd: Pd(4, 1) }),
                }
            }
            (5, 1) => {
                s.mode = Mode::On {
                    beams: Some(BeamPattern::Whirl),
                    spiders: Some(SpiderPattern::Alternate { pd: Pd(4, 1) }),
                }
            }
            (6, 1) => {
                s.mode = Mode::On {
                    beams: Some(BeamPattern::Twisting),
                    spiders: Some(SpiderPattern::Alternate { pd: Pd(4, 1) }),
                }
            }
            ...
            (3, 2) => s.mode = Mode::AutoBeat { pd: Pd(4, 1), r: (0.2..1.0).into(), beam: BeamPattern::WaveY },
            (4, 2) => s.mode = Mode::AutoBeat { pd: Pd(4, 1), r: (0.2..1.0).into(), beam: BeamPattern::UpDownWave },
            (5, 2) => s.mode = Mode::AutoBeat { pd: Pd(4, 1), r: (0.2..1.0).into(), beam: BeamPattern::Whirl },
            (6, 2) => s.mode = Mode::AutoBeat { pd: Pd(4, 1), r: (0.2..1.0).into(), beam: BeamPattern::Twisting },
            ...
            (2, 3) => s.mode = Mode::AutoBeat { pd: Pd(2, 1), r: (0.2..1.0).into(), beam: BeamPattern::RaisingBeams },
            (3, 3) => s.mode = Mode::AutoBeat { pd: Pd(2, 1), r: (0.2..1.0).into(), beam: BeamPattern::WaveY },
            (4, 3) => s.mode = Mode::AutoBeat { pd: Pd(2, 1), r: (0.2..1.0).into(), beam: BeamPattern::UpDownWave },
            (5, 3) => s.mode = Mode::AutoBeat { pd: Pd(2, 1), r: (0.2..1.0).into(), beam: BeamPattern::Whirl },
            (6, 3) => s.mode = Mode::AutoBeat { pd: Pd(2, 1), r: (0.2..1.0).into(), beam: BeamPattern::Twisting },

            // y=4: Pd(1, 1) patterns
            (1, 4) => s.mode = Mode::AutoBeat { pd: Pd(1, 1), r: (0.2..1.0).into(), beam: BeamPattern::Square },
            (2, 4) => s.mode = Mode::AutoBeat { pd: Pd(1, 1), r: (0.2..1.0).into(), beam: BeamPattern::RaisingBeams },
            (3, 4) => s.mode = Mode::AutoBeat { pd: Pd(1, 1), r: (0.2..1.0).into(), beam: BeamPattern::WaveY },
            (4, 4) => s.mode = Mode::AutoBeat { pd: Pd(1, 1), r: (0.2..1.0).into(), beam: BeamPattern::UpDownWave },
            (5, 4) => s.mode = Mode::AutoBeat { pd: Pd(1, 1), r: (0.2..1.0).into(), beam: BeamPattern::Whirl },
            (6, 4) => s.mode = Mode::AutoBeat { pd: Pd(1, 1), r: (0.2..1.0).into(), beam: BeamPattern::Twisting },
            ...
            (4, 5) => s.mode = Mode::ChaseColor { pd: Pd(1, 2), beam: BeamPattern::Twisting },
            ...
            // Colorz
            // Sidez
            (0, 6) => s.colors = s.palette.colors(0, 0),
            (7, 6) => s.colors = s.palette.colors(0, 1),
            // Primariez
            (1, 6) => s.colors = s.palette.colors(1, 0),
            (1, 7) => s.colors = s.palette.colors(1, 1),
            (2, 6) => s.colors = s.palette.colors(2, 0),
            (2, 7) => s.colors = s.palette.colors(2, 1),
            (3, 6) => s.colors = s.palette.colors(3, 0),
            (3, 7) => s.colors = s.palette.colors(3, 1),
            // nBluez
            (4, 6) => s.colors = s.palette.colors(4, 0),
            (4, 7) => s.colors = s.palette.colors(4, 1),
            // Rainbowz
            (5, 6) => s.colors = s.palette.colors(5, 0),
            (5, 7) => s.colors = s.palette.colors(5, 1),
            (6, 6) => s.colors = s.palette.colors(6, 0),
            (6, 7) => s.colors = s.palette.colors(6, 1),
...
pub fn on_ctrl(s: &mut State, l: &mut Lights, ctrl: &mut Midi<LaunchControlXL>, input: launch_control_xl::Input) {
    ...
    match input {
        ...
        Input::Slider(1, fr) => s.duty = fr,
        Input::Slider(2, fr) => s.phi_mul2 = fr.lerp(0.25..4.0),
        ...
        Input::Control(0, true) => s.palette = Palette::Standard,
        Input::Control(1, true) => s.palette = Palette::Trance,
        Input::Control(2, true) => s.palette = Palette::Redhot,
        Input::Control(3, true) => s.palette = Palette::Ashton1,
        Input::Control(4, true) => s.palette = Palette::Ashton2,
        Input::Control(5, true) => s.palette = Palette::Skullsystem,
